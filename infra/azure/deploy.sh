#!/usr/bin/env bash
#
# Deploy Frugal to the Azure VM.
#
#   ./deploy.sh <public-ip>
#
# Builds nothing locally and pushes no images. The VM builds from source, which
# on a B1s is slow (several minutes for the worker image) but avoids a
# registry: Azure Container Registry's Basic tier is ~$5/month and bills
# whether or not anything pulls from it, which on a fixed student credit is
# more than half the VM. For a single host, building in place is the cheaper
# trade.
#
# Secrets are not in this script and not in the repository. They live in
# /opt/frugal/.env on the VM, created once by hand -- see RUNBOOK.md section 2.
...
The VM is probably fine -- check whether it is serving HTTPS:

  curl -fsS https://${HOST}/health/ready

If that answers, only SSH is blocked, and the cause is almost always that your
ISP moved your address since the network security group was last updated. One
command fixes it:

  ./allow-my-ip.sh

If port 443 is also silent, the VM really is down, deallocated, or destroyed.
A student subscription that has spent its credit deallocates VMs rather than
billing for them, so check that first:
...
ssh -o ConnectTimeout=10 "${REMOTE}" 'test -f /opt/frugal/.bootstrapped' || {
  echo "Connected, but the VM has not finished cloud-init." >&2
...
ssh "${REMOTE}" "test -f ${APP_DIR}/.env" || {
  cat >&2 <<EOF
No ${APP_DIR}/.env on the VM.

Secrets are never deployed from here -- cloud-init custom_data is readable by
...
not a place for credentials. Create it once by hand:
  ...
  ssh ${REMOTE}
  sudo -u ${SSH_USER} tee ${APP_DIR}/.env >/dev/null <<'ENVFILE'
  ...
  DOMAIN=<your domain, for TLS>
  CORS_ORIGINS=https://<your-app>.onrender.com
  ENVFILE
  chmod 600 ${APP_DIR}/.env

Note what is absent: no storage credential of any kind. The VM authenticates
to Blob Storage as its managed identity, and the storage account has shared
keys disabled, so there is nothing to put here even if you wanted to.
...
# --- ship the source --------------------------------------------------------
# rsync rather than git clone: no deploy key on the VM, and no requirement that
# the commit be pushed before it can be tested.
...
# Alembic through Neon's pooler can fail partway and leave the schema in a
# state no migration describes.

say "Applying migrations"
ssh "${REMOTE}" bash -euo pipefail <<'REMOTE_SCRIPT'
cd /opt/frugal/infra/azure
...
echo "Check it from here, not from the VM -- that also proves the network"
echo "security group, DNS and TLS are right, which a request from localhost"
echo "does not:"
